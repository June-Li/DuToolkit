...
from MODELALG.REC.CRNN.CRNNv0.networks import build_model, build_loss
from MODELALG.REC.CRNN.CRNNv0.datasets import build_dataloader
from MODELALG.REC.CRNN.CRNNv0.utils import (
    get_logger,
    weight_init,
    load_checkpoint,
    save_checkpoint,
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=cur_dir + "/config/rec_train_config.py",
...
def build_scheduler(optimizer, config):
    ...
    if sch_type == "LambdaLR":
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode="max", factor=0.1, patience=3, verbose=True, threshold=1e-4
        )
    return scheduler
...
def evaluate(net, val_loader, loss_func, to_use_device, logger, converter, metric):
    ...
    net.eval()
    nums = 0
    result_dict = {"eval_loss": 0.0, "eval_acc": 0.0, "norm_edit_dis": 0.0}
    show_str = []
    with torch.no_grad():
...
def train(
    ...
    from MODELALG.REC.CRNN.CRNNv0.metrics import RecMetric
    ...
    try:
        for epoch in range(start_epoch, train_options["epochs"]):  # traverse each epoch
            ...
            for i, batch_data in enumerate(
                train_loader
            ):  # traverse each batch in the epoch
                ...
                if (i + 1) % train_options["print_interval"] == 0:
                    ...
                    start = time.time()
                if (i + 1) >= train_options["val_interval"] and (i + 1) % train_options[
                    "val_interval"
                ] == 0:
                    ...
                    if train_options["ckpt_save_type"] == "HighestAcc":
                        ...
                        if eval_dict["eval_acc"] > best_model["eval_acc"]:
                            ...
                            global_state["global_step"] = global_step
                            net_save_path = (
                                f"{train_options['checkpoint_save_dir']}/best.pth"
                            )
                            save_checkpoint(
                                net_save_path,
                                net,
                        ...
                        train_options["ckpt_save_type"] == "FixedEpochStep"
                        and epoch % train_options["ckpt_save_epoch"] == 0
                    ):
                        shutil.copy(
                            net_save_path,
...
def main():
    ...
    logger = get_logger(
        "torchocr",
        log_file=os.path.join(cfg.train_options["checkpoint_save_dir"], "train.log"),
    )
    ...
    logger.info(cfg)
    # ===>
    to_use_device = select_device(train_options["device"])
