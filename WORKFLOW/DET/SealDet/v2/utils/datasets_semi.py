...
from MODELALG.DET.YOLO.SSLYOLOv5.utils.torch_utils import torch_distributed_zero_first
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources="streams.txt", img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.fps = (
                cap.get(cv2.CAP_PROP_FPS) % 100
            ) or 30.0  # assume 30 FPS if cap gets 0 FPS
            self.frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

            _, self.imgs[i] = cap.read()  # guarantee first frame
            thread = Thread(target=self.update, args=([i, cap]), daemon=True)
            print(
                f" success ({f'{self.frames} frames ' if self.frames else ''}{w}x{h} at {self.fps:.2f} FPS)."
            )
            thread.start()
        print("")  # newline
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        if "/train" in path:
            self.semi_img_files = []
            image_name_list = os.listdir(path.replace("/images/", "/un_images/"))
            for image_name in image_name_list:
                if not os.path.exists(
                    path.replace("/images/", "/un_labels/")
                ...
                if (
                    ...
                    self.semi_img_files.append(
                        path.replace("/images/", "/un_images/") + image_name
                    )
            print("use unlabeled predict label num: ", len(self.semi_img_files))
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            if self.train_type == "SL":
                img, labels = load_mosaic_SL(self, index)
            elif self.train_type == "SSL":
                img, labels = load_mosaic_SSL(self, index)
            else:
                raise ValueError
            shapes = None
            ...
            if random.random() < hyp["mixup"]:
                if self.train_type == "SL":
                    img2, labels2 = load_mosaic_SL(self, random.randint(0, self.n - 1))
                elif self.train_type == "SSL":
                    img2, labels2 = load_mosaic_SSL(self, random.randint(0, self.n - 1))
                else:
                    raise
...
def load_semi_image(self, index):
    path = self.semi_img_files[index]
    img = cv2.imread(path)  # BGR
    assert img is not None, "Image Not Found " + path
    h0, w0 = img.shape[:2]  # orig hw
...
def load_mosaic_SSL(self, index):
    ...
    if len(self.semi_img_files) >= 2:
        semi_index = [random.randint(0, len(self.semi_img_files) - 1) for _ in range(2)]
    else:
        semi_index = []
    sl_index = [self.indices[random.randint(0, self.n - 1)] for _ in range(1)]
    indices = [index] + sl_index + semi_index
    # indices = [index] + semi_index
    for i, index in enumerate(indices):
        if i <= 1 + len(sl_index) - 1:
            # if i == 0:
            img, _, (h, w) = load_image(self, index)
            x = self.labels[index]
        else:
            img, _, (h, w) = load_semi_image(self, index)
            x = []
            lines = open(
                self.semi_img_files[index]
                .replace("/un_images/", "/un_labels/")
                .replace(".jpg", ".txt"),
                "r",
            ).readlines()
            for line in lines:
                line = line.rstrip("\n").rstrip(" ").lstrip(" ").split(" ")
                x.append([int(line[0])] + [float(i) for i in line[1:]])
    ...
    #     cv2.rectangle(show_img, (box[1], box[2]), (box[3], box[4]), (0, 0, 255), thickness=2)
    # cv2.imwrite('buffer/a.jpg', show_img)
    return img4, labels4
