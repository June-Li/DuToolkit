...
from MODELALG.DET.YOLO.SSLYOLOv5.models.yolo import Model
from MODELALG.DET.YOLO.SSLYOLOv5.utils.autoanchor import check_anchors
...
from MODELALG.DET.YOLO.SSLYOLOv5.utils.google_utils import attempt_download
from MODELALG.DET.YOLO.SSLYOLOv5.utils.loss import ComputeLoss
from MODELALG.DET.YOLO.SSLYOLOv5.utils.plots import (
    plot_images,
    plot_labels,
    plot_results,
    plot_evolution,
)
from MODELALG.DET.YOLO.SSLYOLOv5.utils.torch_utils import (
    ModelEMA,
    select_device,
    intersect_dicts,
...
from MODELALG.DET.YOLO.SSLYOLOv5.utils.wandb_logging.wandb_utils import (
    WandbLogger,
    check_wandb_resume,
)
from WORKFLOW.DET.SealDet.v2 import test  # import test.py to get mAP after each epoch
from WORKFLOW.DET.SealDet.v2.utils.datasets_semi import create_dataloader
from WORKFLOW.DET.SealDet.v2.utils.update_unlabel import gen_label_unlabel
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(
        ...
        if pp_flag and opt.train_type == "SSL":
            gen_label_unlabel(str(wdir), opt, opt.unlabels_img_path)
            pp_flag = False
        dataloader, dataset = create_dataloader(
            train_path,
            imgsz,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_type", type=str, default="SSL", help="可选参数为SL、SSL")
    parser.add_argument("--update_unlabel_conf", type=float, default=0.7)
    parser.add_argument(
        "--weights",
        type=str,
        default=root_dir
        + "/MODEL/DET/YOLO/SSLYOLOv5/SealDet/20210720/runs/train/exp3/weights/best.pt",
        help="initial weights path",
    )  # root_dir + '/MODEL/DET/YOLO/SSLYOLOv5/SealDet/20210716//best.pt'
    parser.add_argument(
        "--cfg",
        type=str,
        default=root_dir + "/MODELALG/DET/YOLO/SSLYOLOv5/models/hub/yolov5l6.yaml",
    ...
    parser.add_argument(
        "--unlabels_img_path",
        type=str,
        default=root_dir + "/DATASETS/DET/SealDet/v1/un_images/train/",
        help="Image path without label",
    )
    parser.add_argument(
        "--hyp",
        type=str,
    ...
    parser.add_argument(
        "--device", default="2,3", help="cuda device, i.e. 0 or 0,1,2,3 or cpu"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--project",
        default=root_dir + "/MODEL/DET/YOLO/SSLYOLOv5/SealDet/20210720/runs/train/",
