...
sys.path.append(os.path.abspath(os.path.join(root_dir, "MODELALG/DET/YOLO/SSLYOLOv3/")))
...
from MODELALG.DET.YOLO.SSLYOLOv3.models.experimental import attempt_load
from MODELALG.DET.YOLO.SSLYOLOv3.models.yolo import Model
from MODELALG.DET.YOLO.SSLYOLOv3.utils.autoanchor import check_anchors
from MODELALG.DET.YOLO.SSLYOLOv3.utils.general import (
...
from MODELALG.DET.YOLO.SSLYOLOv3.utils.google_utils import attempt_download
from MODELALG.DET.YOLO.SSLYOLOv3.utils.loss import compute_loss
from MODELALG.DET.YOLO.SSLYOLOv3.utils.plots import (
    plot_images,
    plot_labels,
    plot_results,
    plot_evolution,
)
from MODELALG.DET.YOLO.SSLYOLOv3.utils.torch_utils import (
    ModelEMA,
    select_device,
    intersect_dicts,
    torch_distributed_zero_first,
)
from WORKFLOW.DET.SealDet.v0 import test  # import test.py to get mAP after each epoch
from WORKFLOW.DET.SealDet.v0.utils.datasets_semi import create_dataloader
from WORKFLOW.DET.SealDet.v0.utils.update_unlabel import gen_label_unlabel
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # Trainloader
    dataloader_, dataset_ = create_dataloader(
        train_path,
        imgsz,
        batch_size,
        gs,
    ...
    mlc = np.concatenate(dataset_.labels, 0)[:, 0].max()  # max label class
    nb = len(dataloader_)  # number of batches
    assert (
        mlc < nc
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            labels = np.concatenate(dataset_.labels, 0)
            c = torch.tensor(labels[:, 0])  # classes
            # cf = torch.bincount(c.long(), minlength=nc) + 1.  # frequency
            # model._initialize_biases(cf.to(device))
            if plots:
                Thread(
                    target=plot_labels, args=(labels, save_dir, loggers), daemon=True
                ).start()
                if tb_writer:
            ...
            if not opt.noautoanchor:
                check_anchors(dataset_, model=model, thr=hyp["anchor_t"], imgsz=imgsz)
    ...
    model.class_weights = labels_to_class_weights(dataset_.labels, nc).to(
        device
    ...
    logger.info(
        ...
        % (imgsz, imgsz_test, dataloader_.num_workers, save_dir, epochs)
    )
    pp_flag = True
    for epoch in range(
        start_epoch, epochs
        ...
        if pp_flag:
            gen_label_unlabel(
                str(wdir), opt, root_dir + "/DATASETS/DET/SealDet/v1/un_images/train/"
            )
            pp_flag = False
    ...
    if rank in [-1, 0]:
        ...
        for f in [last, best]:
            if f.exists():  # is *.pt
                ...
                os.system(
                    "gsutil cp %s gs://%s/weights" % (f, opt.bucket)
...
if __name__ == "__main__":
    # python train.py --data data/ocr_table.yaml --cfg yolov3.yaml --weights '' --batch-size 16
    parser = argparse.ArgumentParser()
    parser.add_argument("--train_type", type=str, default="SL", help="可选参数为SL、SSL")
    parser.add_argument(
        "--weights",
        type=str,
        default=root_dir
        + "/MODEL/DET/YOLO/SSLYOLOv3/SealDet/20210717/runs/train/exp/weights/last.pt",
        help="initial weights path",
    )
    parser.add_argument("--update_unlabel_conf", type=float, default=0.3)
    parser.add_argument(
        "--cfg",
        type=str,
        default=root_dir + "/MODELALG/DET/YOLO/SSLYOLOv3/models/yolov3.yaml",
        help="model.yaml path",
    )
    parser.add_argument(
        "--data",
        type=str,
        default=cur_dir + "/config/ocr_seal_semi.yaml",
        help="data.yaml path",
    )
    parser.add_argument(
        "--hyp",
        type=str,
        default=cur_dir + "/config/hyp.scratch.yaml",
        help="hyperparameters path",
    )
    parser.add_argument("--epochs", type=int, default=3000)
    parser.add_argument(
        "--batch-size", type=int, default=24, help="total batch size for all GPUs"
    )
    parser.add_argument(
        "--img-size",
        nargs="+",
    ...
    parser.add_argument(
        "--project",
        default=root_dir + "/MODEL/DET/YOLO/SSLYOLOv3/SealDet/20210717/runs/train/",
    ...
    if opt.train_type == "SSL":
        # opt.weights = './runs/semi/exp/weights/last.pt'
        print("train type SSL")
    elif opt.train_type == "SL":
        # opt.weights = './weights/yolov3.pt'
        print("train type SL")
    else:
