...
from MODELALG.DET.YOLO.SSLYOLOv3.utils.datasets import (
    LoadStreams,
    LoadImages,
    letterbox,
)
...
from MODELALG.DET.YOLO.SSLYOLOv3.utils.plots import plot_one_box
    ...
    def __init__(self, args, options):
        with torch.no_grad():
            ...
            if os.path.exists(self.weights):
                ...
                self.model = attempt_load(
                    options.weights, map_location=self.device
                )  # load FP32 model
            ...
            self.colors = [
                [random.randint(0, 255) for _ in range(3)] for _ in self.names
            ]
...
def gen_label_unlabel(model_path, options, path):
    ...
    for index_one, image_name in enumerate(image_name_list):
        ...
        for index, box in enumerate(boxes):
            ...
            x_center, y_center = (box[0] + box[2]) / 2 / img_w, (
                box[1] + box[3]
            ) / 2 / img_h
            w, h = abs(box[0] - box[2]) / img_w, abs(box[1] - box[3]) / img_h
            out_str += (
                "0 "
                + " ".join([str(round(i, 5)) for i in [x_center, y_center, w, h]])
                + "\n"
            )
            out_str_list.append(
                "0 "
        ...
        if os.path.exists(
            image_path.replace("/un_images/", "/un_labels/").replace(".jpg", ".txt")
        ):
            os.remove(
                image_path.replace("/un_images/", "/un_labels/").replace(".jpg", ".txt")
            )
        ...
        if options.train_type == "SSL":
            open(
                image_path.replace("/un_images/", "/un_labels/").replace(
                    ".jpg", ".txt"
                ),
                "w",
            ).writelines(out_str)
