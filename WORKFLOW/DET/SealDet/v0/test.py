...
from MODELALG.DET.YOLO.SSLYOLOv3.utils.datasets import create_dataloader
...
from MODELALG.DET.YOLO.SSLYOLOv3.utils.metrics import ap_per_class, ConfusionMatrix
...
def test(
    ...
    if not training:
        ...
        dataloader = create_dataloader(
            model, path, imgsz, batch_size, model.stride.max(), opt, pad=0.5, rect=True
        )[0]
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--weights', type=str, default='./runs/semi/b/weights/last.pt', help='model.pt path(s)')
    parser.add_argument(
        "--weights",
        type=str,
        default="./runs/semi/save/20210514/exp/weights/last.pt",
        help="model.pt path(s)",
    )
    parser.add_argument(
        "--data", type=str, default="data/ocr_table_semi.yaml", help="*.data path"
    )
    parser.add_argument(
    ...
    opt.save_json |= opt.data.endswith("ocr_table_semi.yaml")
