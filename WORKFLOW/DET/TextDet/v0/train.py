...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=cur_dir + "/config/det_train_db_config.py",
...
def adjust_learning_rate(
    optimizer, base_lr, iter, all_iters, factor, warmup_iters=0, warmup_factor=1.0 / 3
):
    """
    带 warmup 的学习率衰减
    :param optimizer: 优化器
...
def train(
    ...
    try:
        for epoch in range(start_epoch, train_options["epochs"]):  # traverse each epoch
            ...
            if epoch % train_options["val_interval"] == 0:
                global_state["start_epoch"] = epoch
                global_state["best_model"] = best_model
                global_state["global_step"] = global_step
                ...
                if train_options["ckpt_save_type"] == "HighestAcc":
                    # val
                    eval_dict = evaluate(
                        net, eval_loader, to_use_device, logger, post_process, metric
                    )
...
def main():
    ...
    to_use_device = torch.device(
        ...
        if torch.cuda.is_available() and ("cuda" in train_options["device"])
        else "cpu"
    )
    ...
    optimizer = build_optimizer(params_to_train, cfg["optimizer"])  # net.parameters()

    # ===> whether to resume from checkpoint
    net = nn.DataParallel(net)
    resume_from = train_options["resume_from"]
    if resume_from:
        net, _, _ = load_checkpoint(
            net,
            resume_from,
            to_use_device,
            optimizer,
