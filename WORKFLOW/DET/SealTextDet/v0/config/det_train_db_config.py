...
config.train_options = {
    # for train
    # 继续训练地址  /workspace/JuneLi/bbtv/SensedealImgAlg/MODEL/DET/DB/DBv0/TextDet/20210601/best.pt
    # /workspace/JuneLi/bbtv/SensedealImgAlg/WORKFLOW/DET/TextDet/v0/weights/ch_pre_det_server_db_res50.pth
    # /workspace/JuneLi/bbtv/SensedealImgAlg/WORKFLOW/DET/TextDet/v0/output/DBNet/checkpoint/latest.pth
    "resume_from": "/workspace/JuneLi/bbtv/SensedealImgAlg/MODEL/DET/DB/DBv0/TextDet/20210601/best.pt",
    "third_party_name": "",  # 加载paddle模型可选
    "checkpoint_save_dir": f"./output/{config.exp_name}/checkpoint",  # 模型保存地址，log文件也保存在这里
    "device": "2,3",  # gpu的索引号，或者cpu
    "epochs": 1200,
    "fine_tune_stage": ["head"],  # 'backbone', 'neck',
...
config.dataset = {
    "train": {
        "dataset": {
            "type": "JsonDataset",
            "file": r"/workspace/JuneLi/bbtv/Datasets/paddleocr推荐的中文训练数据集/ICDAR2019-ArT/my_train/v2/train_labels.json",
    ...
    "eval": {
        "dataset": {
            "type": "JsonDataset",
            "file": r"/workspace/JuneLi/bbtv/Datasets/paddleocr推荐的中文训练数据集/ICDAR2019-ArT/my_train/v2/val_labels.json",
            "mean": [0.485, 0.456, 0.406],
            "std": [0.229, 0.224, 0.225],
            "pre_processes": [
                {
