...
from WORKFLOW.OTHER.surya.surya.detection import batch_detection
from WORKFLOW.OTHER.surya.surya.postprocessing.heatmap import (
    keep_largest_boxes,
    get_and_clean_boxes,
    get_detected_boxes,
)
from WORKFLOW.OTHER.surya.surya.schema import (
    LayoutResult,
    LayoutBox,
    TextDetectionResult,
)
...
def get_regions_from_detection_result(
    ...
    for bbox in vertical_line_bboxes:
        ...
        logits[
            :, vert_bbox[1] : vert_bbox[3], vert_bbox[0] : vert_bbox[2]
        ] = 0  # zero out where the column lines are
    ...
    logits[segment_assignment != np.arange(logits.shape[0]).reshape(-1, 1, 1)] = 0
    # print("*" * 99, time.time() - starttime)
    ...
    for heatmap_idx in range(1, len(id2label)):  # Skip the blank class
        ...
        for bbox in bboxes:
            detected_boxes.append(
                LayoutBox(
                    polygon=bbox.polygon, label=id2label[heatmap_idx], confidence=1
                )
            )
    # print("*" * 99, time.time() - starttime)
    ...
    for thresh in [0.5, 0.4]:
        for bbox_idx, bbox in enumerate(detected_boxes):
            for line_idx, line_bbox in enumerate(line_bboxes):
                if (
                    line_bbox.intersection_pct(bbox) > thresh
                    and line_idx not in used_lines
                ):
    ...
    for i in range(5):  # Up to 5 rounds of merging
        ...
        for bbox_idx, bbox in enumerate(new_boxes):
            ...
            for bbox_idx2, bbox2 in enumerate(new_boxes):
                if (
                    bbox2.label != "Table"
                    or bbox_idx2 in to_remove
                    or bbox_idx == bbox_idx2
                ):
                    continue
    ...
    unused_lines = [
        line for idx, line in enumerate(line_bboxes) if idx not in used_lines
    ]
    ...
    detected_boxes = [
        bbox for idx, bbox in enumerate(detected_boxes) if idx not in contained_bbox
    ]
...
def get_regions(
    heatmaps: List[np.ndarray], orig_size, id2label, segment_assignment
) -> List[LayoutBox]:
    bboxes = []
...
def parallel_get_regions(
    heatmaps: List[np.ndarray], orig_size, id2label, detection_results=None
) -> LayoutResult:
    logits = np.stack(heatmaps, axis=0)
    segment_assignment = logits.argmax(axis=0)
    if detection_results is not None:
        bboxes = get_regions_from_detection_result(
            detection_results, heatmaps, orig_size, id2label, segment_assignment
        )
    else:
...
def batch_layout_detection(
    ...
    # print("layout predict use time: ", time.time() - starttime)
    ...
    ):  # Ensures we don't parallelize with streamlit or too few images
        for i in range(len(images)):
            result = parallel_get_regions(
                preds[i],
                orig_sizes[i],
                id2label,
    ...
    # print("后处理 use time: ", time.time() - starttime)
    return results
