...
from WORKFLOW.OTHER.surya.surya.postprocessing.util import get_line_angle, rescale_bbox
from WORKFLOW.OTHER.surya.surya.schema import ColumnLine
...
def get_detected_lines(
    image, slope_tol_deg=2, vertical=False, horizontal=False
) -> List[ColumnLine]:
    ...
    if lines is not None:
        for line in lines:
            ...
            row = ColumnLine(
                bbox=bbox, vertical=vertical_line, horizontal=horizontal_line
            )
...
def get_vertical_lines(
    image, processor_size, image_size, divisor=20, x_tolerance=40, y_tolerance=20
) -> List[ColumnLine]:
    ...
    vertical_lines = [
        line for i, line in enumerate(vertical_lines) if i not in to_remove
    ]
