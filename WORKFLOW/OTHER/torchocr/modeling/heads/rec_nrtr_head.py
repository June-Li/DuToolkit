...
import numpy as np
from WORKFLOW.OTHER.torchocr.modeling.backbones.rec_svtrnet import Mlp
...
class Transformer(nn.Module):
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(1, self.max_len):
            ...
            tgt_mask = self.generate_square_subsequent_mask(
                dec_seq_embed.size(1), dec_seq_embed.device
            )
            tgt = dec_seq_embed
            for decoder_layer in self.decoder:
                tgt = decoder_layer(tgt, memory, self_mask=tgt_mask)
    ...
    def forward_beam(self, images):
        ...
        return [
            torch.from_numpy(np.array(result_hyp), dtype=torch.int64),
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key=None, attn_mask=None):
        ...
        if self.self_attn:
            qkv = self.qkv(query)
            qkv = qkv.reshape(
                (qkv.shape[0], qN, 3, self.num_heads, self.head_dim)
            ).permute((2, 0, 3, 1, 4))
            q, k, v = qkv[0], qkv[1], qkv[2]
        else:
            kN = key.shape[1]
            q = self.q(query)
            q = q.reshape([q.shape[0], qN, self.num_heads, self.head_dim]).permute(
                [0, 2, 1, 3]
            )
            kv = self.kv(key)
            kv = kv.reshape(
                (kv.shape[0], kN, 2, self.num_heads, self.head_dim)
            ).permute((2, 0, 3, 1, 4))
            k, v = kv[0], kv[1]
