...
from WORKFLOW.OTHER.OCR.v0.utils import extra_var
...
class Pdf2Txt:
    ...
    def apply_one(self, du_hypertxt):
        ...
        if (
            du_hypertxt["configs"]["PROCESSES_CONTROL"]["PagePreprocessModule"][
                "use_pdf_tool_text_repair"
            ]["use_flag"]
            and filepath.split("/")[-1].split(".")[-1] in utils.pdf_suffix
        ):
            if (
                du_hypertxt["configs"]["PROCESSES_CONTROL"]["PagePreprocessModule"][
                    "use_pdf_tool_text_repair"
                ...
                logger.info(
                    " ···-> 使用fitz提取的文本进行修复({})···->".format(
                        filepath.split("/")[-1]
                    )
                )
                du_hypertxt["unscan_text_box_dict"] = (
                    utils.get_unscan_texts_boxes_from_fitz(filepath)
                )
            else:
                logger.info(
                    " ···-> 使用pdfplumber提取的文本进行修复({})···->".format(
                        filepath.split("/")[-1]
                    )
                )
                du_hypertxt["unscan_text_box_dict"] = (
                    utils.get_unscan_texts_boxes_from_pdfplumber(filepath)
                )

        extractor = Extract(
            filepath, pdf, fitz_pdf, du_hypertxt, extra=extra_var.get(None)
        )
        ...
        if used_time > 10.0:
            logger.info("Long time consumed: " + str(used_time))
    ...
    def generate_hypertxt(self, phrases, pages_bbox):
        ...
        maker = MakeChapter(self.configs)
...
class Extract:
    ...
    def __init__(self, filepath, pdf, fitz_pdf, du_hypertxt, extra=None):
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=du_hypertxt["configs"]["CONCURRENT"]["max_workers"]
        )
    ...
    def process_page(
        self, page, page_index, du_hypertxt_ori, is_double_column, double_column_pages
    ):
        # if isinstance(page, str):
        #     with pp.open(page) as file:
        ...
        du_hypertxt = copy.deepcopy(du_hypertxt_ori)
        tables = find_tables(du_hypertxt, page, self.fitz_pdf[page_index], page_index)
        ...
        if (
            ...
            pages_segment_lines.append(left_segment_lines)
            pages_bbox.append(left_bbox)
            page_idx_list.append(page_index + 1)
            pages_segment_lines.append(right_segment_lines)
            pages_bbox.append(right_bbox)
            page_idx_list.append(page_index + 1)
        else:
            # 使用原来的segment_to_lines
            segment_lines = segment_to_lines(segments)
            pages_segment_lines.append(segment_lines)
            pages_bbox.append(page.bbox)
            page_idx_list.append(page_index + 1)
        return pages_segment_lines, pages_bbox, page_idx_list

    def worker(self, args):
        # 解包参数
        page, page_index, du_hypertxt_ori, is_double_column, double_column_pages = args
        return self.process_page(
            page, page_index, du_hypertxt_ori, is_double_column, double_column_pages
        )
    ...
    def extract(self, du_hypertxt, layout):
        ...
        if not du_hypertxt["configs"]["CONCURRENT"]["use_flag"]:
            # 单线程推理 -> start
            logger.info(
                "PDF2TXT正在使用1个线程进行推理({})>>>>>>".format(
                    du_hypertxt["filename"]
                )
            )
            for page_index, page in enumerate(self.pdf.pages):
                pages_segment_lines, pages_bbox, page_idx_list = self.process_page(
                    page, page_index, du_hypertxt, is_double_column, double_column_pages
                )
                self.pages_segment_lines.extend(pages_segment_lines)
                self.pages_bbox.extend(pages_bbox)
                self.page_idx_list.extend(page_idx_list)
            # 单线程推理 -> end
        else:  # if du_hypertxt["configs"]["CONCURRENT"]["multi_type"] == "threading":
            # 多线程 -> start
            logger.info(
                "PDF2TXT正在使用{}个线程进行推理({})>>>>>>".format(
                    du_hypertxt["configs"]["CONCURRENT"]["max_workers"],
                    du_hypertxt["filename"],
                )
            )
            args_list = [
            ...
            # ) as executor:
            #     total_result = list(
            #         executor.map(lambda x: self.process_page(*x), args_list)
            #     )
            total_result = list(
                self.executor.map(lambda x: self.process_page(*x), args_list)
            )
            for elem in total_result:
                self.pages_segment_lines.extend(elem[0])
                self.pages_bbox.extend(elem[1])
                self.page_idx_list.extend(elem[2])
            # 多线程 -> end
        # else:
        #     # 多进程推理 -> start
        #     logger.info(
        ...
        #     )
        #     with Pool(
        #         processes=du_hypertxt["configs"]["CONCURRENT"]["max_workers"]
        #     ) as pool:
        #         args_list = [
        #             (
        #                 self.filepath,
        ...
        #             for page_index, page in enumerate(self.pdf.pages)
        #         ]
        #         total_result = pool.map(self.worker, args_list)
        #     for elem in total_result:
        #         self.pages_segment_lines.extend(elem[0])
        #         self.pages_bbox.extend(elem[1])
        #         self.page_idx_list.extend(elem[2])
