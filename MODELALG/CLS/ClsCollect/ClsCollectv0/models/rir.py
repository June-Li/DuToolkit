...
class ResnetInit(nn.Module):
    def __init__(self, in_channel, out_channel, stride):
        ...
        self.transient_stream_conv_across = nn.Conv2d(
            in_channel, out_channel, 3, padding=1, stride=stride
        )
    ...
    def forward(self, x):
        ...
        x_residual = self.residual_bn_relu(
            residual_r_r + transient_t_r + residual_shortcut
        )
...
class RiRBlock(nn.Module):
    ...
    # (Figure 1d)."""
    def _make_layers(
        self, in_channel, out_channel, layer_num, stride, layer=ResnetInit
    ):
        strides = [stride] + [1] * (layer_num - 1)
        layers = nn.Sequential()
        for index, s in enumerate(strides):
            layers.add_module(
                "generalized layers{}".format(index), layer(in_channel, out_channel, s)
            )
...
class ResnetInResneet(nn.Module):
    def __init__(self, num_classes=100):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                384, num_classes, kernel_size=3, stride=2
            ),  # without this convolution, loss will soon be nan
