...
class LocalizationNetwork(nn.Module):
    ...
    def load_3rd_state_dict(self, _3rd_name, _state_dict):
        ...
        if _3rd_name == "paddle":
            ...
            for i in range(4):
                to_load_state_dict[f"conv.{i * 4}.weight"] = torch.Tensor(
                    _state_dict[f"loc_conv{i}_weights"]
                )
                to_load_state_dict[f"conv.{i * 4 + 1}.weight"] = torch.Tensor(
                    _state_dict[f"bn_loc_conv{i}_scale"]
                )
                to_load_state_dict[f"conv.{i * 4 + 1}.bias"] = torch.Tensor(
                    _state_dict[f"bn_loc_conv{i}_offset"]
                )
                to_load_state_dict[f"conv.{i * 4 + 1}.running_mean"] = torch.Tensor(
                    _state_dict[f"bn_loc_conv{i}_mean"]
                )
                to_load_state_dict[f"conv.{i * 4 + 1}.running_var"] = torch.Tensor(
                    _state_dict[f"bn_loc_conv{i}_variance"]
                )
            to_load_state_dict[f"localization_fc1.0.weight"] = torch.Tensor(
                _state_dict["loc_fc1_w"].T
            )
            to_load_state_dict[f"localization_fc1.0.bias"] = torch.Tensor(
                _state_dict["loc_fc1.b_0"]
            )
            to_load_state_dict[f"localization_fc2.weight"] = torch.Tensor(
                _state_dict["loc_fc2_w"].T
            )
            to_load_state_dict[f"localization_fc2.bias"] = torch.Tensor(
                _state_dict["loc_fc2_b"]
            )
...
class GridGenerator(nn.Module):
    ...
    def __init__(self, F, I_r_size):
        ...
        self.inv_delta_C = torch.tensor(
            self._build_inv_delta_C(self.F, self.C)
        ).float()  # F+3 x F+3
        self.P_hat = torch.tensor(
            self._build_P_hat(self.F, self.C, self.P)
        ).float()  # n x F+3
    ...
    def build_P_prime(self, batch_C_prime):
        ...
        batch_inv_delta_C = self.inv_delta_C.repeat(batch_size, 1, 1).to(device)
        batch_P_hat = self.P_hat.repeat(batch_size, 1, 1).to(device)
