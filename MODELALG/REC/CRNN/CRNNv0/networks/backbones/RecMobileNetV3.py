...
from MODELALG.REC.CRNN.CRNNv0.networks.CommonModules import ConvBNACT, SEBlock
...
class ResidualUnit(nn.Module):
    def __init__(
        ...
        self.bottleneck_conv = ConvBNACT(
        ...
        if use_se:
            self.se = SEBlock(
                in_channels=num_mid_filter,
                out_channels=num_mid_filter,
                hsigmoid_type="paddle",
            )
        else:
            self.se = None

        self.linear_conv = ConvBNACT(
            in_channels=num_mid_filter,
    ...
    def load_3rd_state_dict(self, _3rd_name, _state, _convolution_index):
        if _3rd_name == "paddle":
            self.expand_conv.load_3rd_state_dict(
                _3rd_name, _state, f"conv{_convolution_index}_expand"
            )
            self.bottleneck_conv.load_3rd_state_dict(
                _3rd_name, _state, f"conv{_convolution_index}_depthwise"
            )
            if self.se is not None:
                self.se.load_3rd_state_dict(
                    _3rd_name, _state, f"conv{_convolution_index}_se"
                )
            self.linear_conv.load_3rd_state_dict(
                _3rd_name, _state, f"conv{_convolution_index}_linear"
            )
        else:
            pass
        pass
