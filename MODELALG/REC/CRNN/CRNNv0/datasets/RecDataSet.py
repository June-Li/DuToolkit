...
from MODELALG.REC.CRNN.CRNNv0.utils.CreateRecAug import (
    cv2pil,
    pil2cv,
    RandomBrightness,
    RandomContrast,
...
class RecDataLoader:
    ...
    def pack(self, batch_data):
        ...
        for i in range(len(batch_data)):
            ...
            img = self.process.normalize_img(
                self.process.width_pad_img(all_same_height_images[i], max_img_w)
            )
    ...
    def __next__(self):
        ...
        if (
            not len(self.queue_2)
            and not len(self.queue_1)
            and self.iteration == len(self.dataset)
        ):
            self.iteration = 0
            self.dataiter = None
            raise StopIteration
...
class RecDataProcess:
    ...
    def width_pad_img(self, _img, _target_width, _pad_value=0):
        ...
        to_return_img = (
            np.ones([_height, _target_width, _channels], dtype=_img.dtype) * _pad_value
        )
