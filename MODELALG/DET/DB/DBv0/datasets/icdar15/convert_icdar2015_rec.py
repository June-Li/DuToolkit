...
if __name__ == "__main__":
    ...
    train_gt_directory = os.path.join(
        icdar2015_directory, "ch4_training_localization_transcription_gt"
    )
    ...
    for m_name, m_image_directory, m_gt_directory in zip(
        ["train", "eval"],
        ...
        m_index = 0
        with open(
            os.path.join(target_directory, m_name + ".txt"), mode="w", encoding="utf-8"
        ) as to_write:
            for m_image_file in os.listdir(m_image_directory):
                m_gt_file = os.path.join(
                    m_gt_directory, "gt_" + os.path.splitext(m_image_file)[0] + ".txt"
                )
                ...
                with open(m_gt_file, mode="r", encoding="utf-8") as to_read:
                    ...
                    for m_line in to_read:
                        ...
                        if not m_line.endswith("###"):
                            ...
                            transcript = " ".join(
                                "".join(coordinates_and_transcript[8:]).split()
                            )
                            if len(transcript) == 0:
                                continue
                            np_coordinates = np.array(
                                [int(_) for _ in coordinates_and_transcript[:8]]
                            ).reshape((-1, 2))
                            ...
                            m_target_roi = np.zeros(
                                (m_height, m_width, m_img.shape[2]), dtype=np.uint8
                            )
                            m_region = np.array(
                                [np_coordinates - [min_x, min_y]], dtype=np.int32
                            )
                            m_target_roi = cv2.fillPoly(
                                m_target_roi, m_region, (255,) * m_img.shape[2]
                            )
                            m_target_roi = cv2.bitwise_and(
                                m_img[min_y : max_y + 1, min_x : max_x + 1, ...],
                                m_target_roi,
                            )
                            target_image_name = f"{m_name}_{m_index}.jpg"
                            cv2.imwrite(
                                os.path.join(target_image_directory, target_image_name),
                                m_target_roi,
                            )
                            m_index += 1
