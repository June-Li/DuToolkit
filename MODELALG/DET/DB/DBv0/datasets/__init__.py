...
def build_dataset(config):
    ...
    assert (
        dataset_type in support_dataset
    ), f"{dataset_type} is not developed yet!, only {support_dataset} are support now"
...
def build_loader(dataset, config):
    ...
    assert (
        dataloader_type in support_loader
    ), f"{dataloader_type} is not developed yet!, only {support_loader} are support now"
    ...
    if "collate_fn" in config:
        ...
        collate_fn = None
    dataloader_class = eval(dataloader_type)(
        dataset=dataset, collate_fn=collate_fn, **config
    )
