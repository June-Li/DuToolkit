...
class HardSigmoid(nn.Module):
    ...
    def forward(self, x):
        if self.type == "paddle":
            x = (1.2 * x).add_(3.0).clamp_(0.0, 6.0).div_(6.0)
        else:
...
class SEBlock(nn.Module):
    ...
    def load_3rd_state_dict(self, _3rd_name, _state, _name_prefix):
        to_load_state_dict = OrderedDict()
        if _3rd_name == "paddle":
            to_load_state_dict["conv1.weight"] = torch.Tensor(
                _state[f"{_name_prefix}_1_weights"]
            )
            to_load_state_dict["conv2.weight"] = torch.Tensor(
                _state[f"{_name_prefix}_2_weights"]
            )
            to_load_state_dict["conv1.bias"] = torch.Tensor(
                _state[f"{_name_prefix}_1_offset"]
            )
            to_load_state_dict["conv2.bias"] = torch.Tensor(
                _state[f"{_name_prefix}_2_offset"]
            )
            self.load_state_dict(to_load_state_dict)
        else:
            pass
