...
class FPN(nn.Module):
    def __init__(self, in_channels, out_channels=256, **kwargs):
        ...
        self.reduce_conv_c2 = ConvBnRelu(
            in_channels[0], out_channels, kernel_size=1, inplace=inplace
        )
        self.reduce_conv_c3 = ConvBnRelu(
            in_channels[1], out_channels, kernel_size=1, inplace=inplace
        )
        self.reduce_conv_c4 = ConvBnRelu(
            in_channels[2], out_channels, kernel_size=1, inplace=inplace
        )
        self.reduce_conv_c5 = ConvBnRelu(
            in_channels[3], out_channels, kernel_size=1, inplace=inplace
        )
        # Smooth layers
        self.smooth_p4 = ConvBnRelu(
            out_channels, out_channels, kernel_size=3, padding=1, inplace=inplace
        )
