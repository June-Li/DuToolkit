...
class DBPostProcess(object):
    ...
    def __call__(self, preds, batch):
        ...
        available_region = np.zeros_like(pred[0], dtype=np.float32)
        np.putmask(available_region, pred[0] > self.thresh, pred[0])
        mask_region = (available_region > 0).astype(np.uint8) * 255
        structure_element = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7))
        segmentation = cv2.morphologyEx(mask_region, cv2.MORPH_CLOSE, structure_element)
