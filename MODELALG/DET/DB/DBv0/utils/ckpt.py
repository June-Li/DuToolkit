...
def load_checkpoint(
    _model, resume_from, to_use_device, _optimizers=None, third_name=None
):
    """
    加载预训练模型
    Args:
    ...
    if not third_name:
        ...
        _model.load_state_dict(state["state_dict"], True)
...
def save_checkpoint_logic(
    total_loss, total_num, min_loss, net, solver, epoch, rec_train_options, logger
):
    """
    根据配置文件保存模型
    Args:
    ...
    if rec_train_options["ckpt_save_type"] == "HighestAcc":
        loss_mean = sum(
            [total_loss[idx] * total_num[idx] for idx in range(len(total_loss))]
        ) / sum(total_num)
        if loss_mean < min_loss:
            min_loss = loss_mean
            save_checkpoint(
                os.path.join(
                    rec_train_options["checkpoint_save_dir"],
