"""Exports a SSLYOLOv5 *.pt model to TorchScript, ONNX, CoreML formats
...
from MODELALG.DET.YOLO.SSLYOLOv5 import models
from MODELALG.DET.YOLO.SSLYOLOv5.models.experimental import attempt_load
from MODELALG.DET.YOLO.SSLYOLOv5.utils.activations import Hardswish, SiLU
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="include formats",
    )
    parser.add_argument(
        "--half", action="store_true", help="FP16 half-precision export"
    )
    parser.add_argument(
        "--inplace", action="store_true", help="set SSLYOLOv5 Detect() inplace=True"
    )
    parser.add_argument("--train", action="store_true", help="model.train() mode")
    parser.add_argument(
        "--optimize", action="store_true", help="optimize TorchScript for mobile"
    )  # TorchScript-only
    parser.add_argument(
    ...
    assert not (
        opt.device.lower() == "cpu" and opt.half
