...
from MODELALG.DET.YOLO.SSLYOLOv5.utils.google_utils import gsutil_getsize
from MODELALG.DET.YOLO.SSLYOLOv5.utils.metrics import fitness
from MODELALG.DET.YOLO.SSLYOLOv5.utils.torch_utils import init_torch_seeds
...
def check_python(minimum="3.7.0", required=True):
    ...
    if required:
        assert (
            result
        ), f"Python {minimum} required by SSLYOLOv5, but Python {current} is currently installed"
    return result
...
def check_requirements(requirements="requirements.txt", exclude=()):
    ...
    for r in requirements:
        try:
            ...
            n += 1
            print(
                f"{prefix} {r} not found and is required by SSLYOLOv5, attempting auto-update..."
            )
