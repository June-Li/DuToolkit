...
from MODELALG.DET.YOLO.YOLOv5.utils.google_utils import gsutil_getsize
from MODELALG.DET.YOLO.YOLOv5.utils.metrics import fitness
from MODELALG.DET.YOLO.YOLOv5.utils.torch_utils import init_torch_seeds
...
def check_python(minimum="3.7.0", required=True):
    ...
    if required:
        assert (
            result
        ), f"Python {minimum} required by YOLOv5, but Python {current} is currently installed"
    return result
