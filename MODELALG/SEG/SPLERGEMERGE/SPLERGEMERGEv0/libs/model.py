...
class Merge_SFCN(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv2d(4, 18, kernel_size=7, stride=1, padding=3)
        self.conv2 = torch.nn.Conv2d(18, 18, kernel_size=7, stride=1, padding=3)
...
class Merge_Block(torch.nn.Module):
    def __init__(self, block_num):
        ...
        self.dil_conv2d_1 = torch.nn.Conv2d(
            self.block_inputs[self.block_num - 1],
            6,
            kernel_size=5,
            dilation=1,
        ...
        # 1x1 convolution for top branch
        self.conv_1x1_top = torch.nn.Conv2d(
            18, self.block_conv1x1_output[self.block_num - 1], kernel_size=1
        )
    ...
    def forward(self, x, grid):
        ...
        for row_id in range(1, len(row_mids)):
            for col_id in range(1, len(col_mids)):
                grid_mean = torch.mean(
                    top_branch_x[
                        :,
                        :,
                        row_mids[row_id - 1] : row_mids[row_id],
                        col_mids[col_id - 1] : col_mids[col_id],
                    ].reshape(
                        batch_size, self.block_conv1x1_output[self.block_num - 1], -1
                    ),
                    dim=2,
                )
                ...
                top_branch_x[
                    ...
                    row_mids[row_id - 1] : row_mids[row_id],
                    col_mids[col_id - 1] : col_mids[col_id],
                ] = grid_mean.unsqueeze(2).unsqueeze(2)

        ################# BOTTOM BRANCH #######################
        bottom_branch_x = self.conv_1x1_bottom(out_feature)

        temp = torch.tensor((), dtype=torch.float32).to(out_feature.device)
        bottom_branch_grid_pools = temp.new_zeros(
            (batch_size, 1, len(row_mids) - 1, len(col_mids) - 1)
        )
...
class MergeModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        D1_probs = (1 / 2) * (up_probs_1[:, :, 1:, :] * down_probs_1[:, :, :-1, :]) + (
            1 / 4
        ) * (up_probs_1[:, :, 1:, :] + down_probs_1[:, :, :-1, :])
        D2_probs = (1 / 2) * (up_probs_2[:, :, 1:, :] * down_probs_2[:, :, :-1, :]) + (
            1 / 4
        ) * (up_probs_2[:, :, 1:, :] + down_probs_2[:, :, :-1, :])
        ...
        R1_probs = (1 / 2) * (
            left_probs_1[:, :, :, 1:] * right_probs_1[:, :, :, :-1]
        ) + (1 / 4) * (left_probs_1[:, :, :, 1:] + right_probs_1[:, :, :, :-1])
        R2_probs = (1 / 2) * (
            left_probs_2[:, :, :, 1:] * right_probs_2[:, :, :, :-1]
        ) + (1 / 4) * (left_probs_2[:, :, :, 1:] + right_probs_2[:, :, :, :-1])
